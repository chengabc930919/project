...
import { getPreference, setPreference } from '@/api/user'
...
const actions = {
  ...
  setSize({ commit }, size) {
    return new Promise(resolve => {
      setPreference({ themeValue: state.themeValue, size }).then(response => {
  ...
  setThemeValue({ commit }, themeValue) {
    return new Promise(resolve => {
      setPreference({ themeValue, size: state.size }).then(response => {
        if (response.code === 20000) {
          const { size, themeValue } = response.data
          commit('SET_SIZE', size)
          commit('SET_THEME_VALUE', themeValue)
